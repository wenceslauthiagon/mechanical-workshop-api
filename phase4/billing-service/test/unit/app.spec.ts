...
function getValidToken(): string {
  return jwt.sign({ sub: 'test-user' }, process.env.JWT_SECRET || 'test-secret');
}
...
describe('App', () => {
  ...
  it('TC0016 - Should return 503 when JWT_SECRET is not configured', async () => {
    delete process.env.JWT_SECRET;
    const { app } = await createApp();
    ...
    expect(response.body).toEqual({ message: 'JWT_SECRET_NOT_CONFIGURED' });
  });

  it('TC0017 - Should connect database when DATABASE_URL is configured', async () => {
