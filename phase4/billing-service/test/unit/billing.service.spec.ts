...
describe('BillingService', () => {
  ...
  describe('generateBudget', () => {
    ...
    it('TC0003 - Should return existing budget for same order id', async () => {
      const oid = randomUUID();
      const first = await service.generateBudget(oid, 500);
      eventEmitter.mockClear();

      const second = await service.generateBudget(oid, 800);

      expect(second.id).toBe(first.id);
      expect(second.estimatedTotal).toBe(500);
      expect(eventEmitter).toHaveBeenCalledWith(
        'event.billing.budget_generated',
        expect.objectContaining({ orderId: oid, budgetId: first.id }),
      );
    });
  });
  ...
  describe('approveOrderPayment', () => {
    it('TC0001 - Should approve order payment from order id', async () => {
      const oid = randomUUID();
      await service.generateBudget(oid, 420);

      const result = await service.approveOrderPayment(oid, 'mp-xyz');

      expect(result.budget.status).toBe('APPROVED');
      expect(result.payment.status).toBe('CONFIRMED');
      expect(result.payment.mercadopagoId).toBe('mp-xyz');
    });

    it('TC0002 - Should throw when budget does not exist for order id', async () => {
      await expect(service.approveOrderPayment(randomUUID())).rejects.toThrow('BUDGET_NOT_FOUND');
    });
  });
});
